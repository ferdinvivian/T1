...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
    if (htim->Instance == TIM11)
    {
			  ...
			  if((prj.Song_Play_Status) && ( Tim11.Motor_Wait_Inc > MOTOR_WAIT))
				{
					STEPPER_MOTOR_ON();
					prj.Motor_status = 1;
				...
				else
				{
					STEPPER_MOTOR_OFF();
					prj.Motor_status = 0;
					if((Tim11.Timer_Inc%(PC13_DELAY*8)) == 0)
						HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_13);
				}
				
				if(HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_10))
				{
					Tim11.Motor_Wait_Inc = 0;
...
void init_function()
	{
		prj.Song_Play_Status = prj.Motor_status = 0;
		STEPPER_MOTOR_OFF();
...
void tim11_function()
	{
		...
		Tim11.DF_STOP+=1;
		Tim11.DF_REPEAT+=1;
		Tim11.Motor_Step_Inc+=1;
		Tim11.Motor_Wait_Inc+=1;
	}
		...
		if(Tim11.DF_REPEAT > 38000)
			{
					...
					if(prj.Loop_song < SONG_LOOP)
							DF_PlayFromStart();
					else
							prj.Loop_song = SONG_LOOP;
			}
}
...
{
	if(Tim11.Motor_Step_Inc > STEP_DELAY)
	{
		Tim11.Motor_Step_Inc = 0;
